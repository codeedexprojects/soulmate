...
class TalkTime(models.Model):
    call_history = models.ForeignKey('user.CallHistory', on_delete=models.CASCADE)
    executive = models.ForeignKey(Executives, on_delete=models.CASCADE)
    duration = models.DurationField()
    coins_deducted = models.DecimalField(max_digits=10, decimal_places=2)

    def __str__(self):
        return f"{self.executive.name} - {self.duration}"

class ExecutiveCallHistory(models.Model):
    executive = models.ForeignKey(Executives, on_delete=models.CASCADE)
    ...
    def __str__(self):
        return f"Call from {self.user.username} to {self.executive.name} - Duration: {self.duration}"
    ...
    def save(self, *args, **kwargs):
        if self.end_time and self.start_time:
            ...
            if self.status not in ['accepted', 'ended']:
                self.status = 'missed'
        super(ExecutiveCallHistory, self).save(*args, **kwargs)
...
class CallRating(models.Model):
    ...
    execallhistory = models.ForeignKey('ExecutiveCallHistory', on_delete=models.CASCADE, related_name="ratings")
